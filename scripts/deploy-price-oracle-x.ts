...
async function deployContract(
  ...
  const value = new BN('2000000000000').toString();  // 2 WND for endowment (ED + storage for prices)
  
  // Set gas limits based on actual usage from previous attempt
  const gasLimit = {
    refTime: new BN('2500000000').toString(),    // 2.5B units (based on previous attempts)
    proofSize: new BN('8000').toString()         // 8K units (based on previous attempts)
  };

  // Set explicit storage deposit limit
  const storageDepositLimit = new BN('2000000000000').toString();  // 2 WND for storage deposit
...
function handleDeploymentCallback(resolve: (address: string) => void, reject: (error: Error) => void) {
  return ({ status, events = [], dispatchError }: any) => {
    ...
    if (status.isFinalized) {
      ...
      fs.writeFileSync(
        path.join(deploymentsDir, 'price-oracle-x.json'),
        JSON.stringify(
          {
      ...
      console.log('Deployment info saved to deployments/price-oracle-x.json');
